...
export class CategoryService {
  ...
  async updateCategory(payload: UpdateCategoryInterface): Promise<void> {
    ...
    const updated_category = await this.#_prisma.category.findFirst({
      where: { id: payload.id },
    });
    if (payload.category_id) {
    ...
    if (payload.name) {
      ...
      await this.#_prisma.translate.delete({
        where: { id: updated_category.name },
      });
      await this.#_prisma.category.update({
        where: { id: payload.id },
    ...
    if (payload.image) {
      const deleteImageFile = await this.#_prisma.category.findFirst({
        where: { id: payload.id },
      });
      await this.#_minio
